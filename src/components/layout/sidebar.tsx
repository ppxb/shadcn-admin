...
export default function LayoutSidebar({ className, isCollapsed, setIsCollapsed }: LayoutSidebarProps) {
  ...
  return (
    <aside
      className={cn(
              `fixed left-0 right-0 top-16 z-50 w-full transition-[width]  md:rounded-2xl md:border md:left-4 md:top-24 md:bottom-4 ${isCollapsed ? 'md:w-16' : 'md:w-72'} bg-gray-50 dark:bg-black`,
              className
      )}
    >
      {/* Mobile Overlay */}
      ...
      <Layout>
        ...
        <Menu
          className={`overflow-auto scrollbar-hide ${navOpened ? 'max-h-[420px]' : 'max-h-0 py-0 md:max-h-screen md:py-2 md:scrollbar-hide'}`}
